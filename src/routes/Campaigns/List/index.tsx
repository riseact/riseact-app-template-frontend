...
const List: FC = () => {
  ...
  const { selected, areAllSelected, toggleSelectAll, toggleSelect } = useSelection<number>(
    data?.campaigns.pageInfo.total,
  );
  ...
  return (
    <PageContainer width="full">
      ...
      <Card.Table>
        ...
        <SelectionBar
          isLoading={loading}
          visualized={data?.campaigns.edges.length}
          total={data?.campaigns.pageInfo.total}
        />
                ...
                <Tbody>
                  {data?.campaigns.edges.map((campaign) => (
                    <Tr
                      ...
                      onClick={() =>
                        navigate(urlJoin(ROUTE.CAMPAIGNS, campaign.node.id!.toString()))
                      }
                      cursor="pointer"
                      _hover={{ bg: 'gray.50' }}
                    >
                      ...
                      <Td>
                        <When condition={campaign.node?.cover?.square}>
                          <Image
                            src={campaign.node?.cover?.square}
                            width="35px"
                            height="35px"
                            objectFit="cover"
                            rounded="md"
